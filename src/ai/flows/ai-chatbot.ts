...
`; // Updated slightly based on new resume data for consistency
...
async (input) => {
    // Basic logic: include if query mentions common resume sections or is general.
    const lowerQuery = input.query.toLowerCase();
    const keywords = ['experience', 'education', 'skills', 'project', 'summary', 'profile', 'technical', 'work', 'qualification', 'background', 'tell me about you', 'who are you'];
    return keywords.some(keyword => lowerQuery.includes(keyword)) || lowerQuery.length < 15; // Include for very short/general queries
  }
);
...
async (input) => {
    // Basic logic: include if query mentions LinkedIn or professional profile context.
    const lowerQuery = input.query.toLowerCase();
    const keywords = ['linkedin', 'professional', 'connect', 'network', 'code and theory', 'y media labs', 'automation anywhere'];
